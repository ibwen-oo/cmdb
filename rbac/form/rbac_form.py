...
from cmdbweb.models import User
...
class RoleModelForm(BootStrapModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for name, field in self.fields.items():
            # print(type(field))
            # 设置多选框高度
            if type(field) == ModelMultipleChoiceField:
...
# 声明向模板中传入的html代码是安全的(否则django不会渲染).
...
class UserModelForm(forms.ModelForm):
    ...
    class Meta:
        model = User
        fields = ['username', 'email', 'phone', 'password', 'confirm_password', 'group']
